...
public class WSIResponseMediationAction extends WSIGatewayAction {

	private String label=null;
	private String path=null;
	private String base=null;

	public WSIResponseMediationAction(JSONObject jsonObject) {
		try {
			label=jsonObject.getString("label").trim();
		} catch(JSONException e) {
			label="WSIGatewayRESTResponseMediation";
		}
		try {
	...
	public void performAction(MbJavaComputeNode node, MbMessageAssembly assy) throws WSIException {
		try {
			...
			MsgUtility.setupCall(assy, "response");
			String requrl=null;
			if(path!=null) requrl=path;
			if(requrl==null) requrl=MsgUtility.getElementAsString(le, "path");
			//path must begin with a /
			if(base==null) throw new WSIException("Base URL is required for response mediations");
			if(requrl==null) throw new WSIException("Cannot resolve path for response mediations");
